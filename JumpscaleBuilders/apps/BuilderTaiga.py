...
class BuilderTaiga(j.baseclasses.builder):
    ...
    def install_deps(self):
        ...
        if not self._done_check("postgresuser"):
            ...
            sudo -u postgres {DIR_BIN}/createuser {TAIGA_USER}
            sudo -u postgres {DIR_BIN}/createdb {TAIGA_USER} -O {TAIGA_USER} --encoding='utf-8' --locale=en_US.utf8 --template=template0
            """
    ...
    def install(self, host, port, protocol="http"):
        ...
        self.frontend_install(host, port, protocol)
        j.sal.fs.writeFile(
